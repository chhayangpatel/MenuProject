  ...
  useEffect(() => {
    ...
    cards.forEach(card => {
      card.addEventListener('click', () => {
        // Find the raw data object embedded somehow, or we can fetch it, 
        // but for simplicity we will rely on a script in the main page dispatching the event
      });
    });
              ...
              <div className="p-6 sm:p-8">
                ...
                {selectedItem.pairsWith && (
                  <div className="bg-[var(--color-primary)]/5 p-4 rounded-xl mb-6 flex gap-3 items-start border border-[var(--color-primary)]/10">
                    <Info size={20} className="text-[var(--color-accent)] shrink-0 mt-0.5" />
                    <div>
                      <p className="font-heading font-semibold text-[var(--color-primary)] text-sm mb-1 uppercase tracking-wide">Pairs well with</p>
                      <p className="font-body text-[var(--color-primary)]/90">{selectedItem.pairsWith}</p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                      {selectedItem.gallery.map((img, i) => (
                        <img key={i} src={img} alt="" className="h-24 w-24 object-cover rounded-lg shrink-0" />
                      ))}
                    </div>
                  </div>
                )}
